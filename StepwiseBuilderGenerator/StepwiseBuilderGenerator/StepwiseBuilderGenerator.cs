...
//     {
//         var provider = context.SyntaxProvider.ForAttributeWithMetadataName(typeof(StepwiseBuilder).FullName!,
//             static (node, _) =>
//                 node
...
//                     .SingleOrDefault(s =>
//                         s.TryFindFirstNode<ObjectCreationExpressionSyntax>()?.Type.TryCast<IdentifierNameSyntax>()
...
//                     .TryCast<ConstructorDeclarationSyntax>()!.Body!.Statements[0]
//                     .TryCast<ExpressionStatementSyntax>()!.Expression
//                     .TryCast<InvocationExpressionSyntax>()
//         ).Where(i => i is not null);
//
//         var usings = provider.Select(static (invocation, _) => invocation?.Ancestors()
//             .Select(a =>
...
//             .ToImmutableHashSet());
//
//         var targetType = provider.Select(static (invocation, _) =>
//             invocation?.Expression
//                 .TryCast<MemberAccessExpressionSyntax>()!.Name
//                 .TryCast<GenericNameSyntax>()!.TypeArgumentList.Arguments[0].ToString());
//
//         var addStepInvocations = provider.Select(static (invocation, _) =>
//             {
...
//
//                 var index = 1;
//                 return addStepInvocations.Reverse().Select(i =>
//                 {
//                     i.Index = index++;
//                     return i;
//                 });
...
//             );
//
//         var classDeclaration = provider.Select(static (invocation, _) =>
//         {
...
//         var compilation =
//             usings.Collect()
//                 .Combine(targetType.Collect())
//                 .Combine(addStepInvocations.Collect())
//                 .Combine(@namespace)
//                 .Combine(classTypeParametersAndConstraints.Collect())
//                 .Combine(className.Collect());
//
